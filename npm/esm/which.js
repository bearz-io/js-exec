 ...
 * @module
 */
import { expand, get, splitPath } from "@bearz/env";
import { basename, extname, isAbsolute, join, resolve } from "@bearz/path";
import { isDir, isDirSync, isFile, isFileSync, readDir, readDirSync } from "@bearz/fs";
import { WIN } from "./globals.js";
import { isNullOrSpace } from "@bearz/strings/is-space";
import { isNullOrEmpty } from "@bearz/strings/is-empty";
import { endsWithFold } from "@bearz/strings/ends-with";
 ...
 */
export function whichSync(fileName, prependPath, useCache = true, debug = false) {
 ...
 * @returns {string | undefined}
 */
export async function which(fileName, prependPath, useCache = true, debug = false) {
    if (isNullOrSpace(fileName)) {
        throw new Error("Argument 'fileName' cannot be null or empty.");
    }
    const rootName = basename(fileName, extname(fileName));
